...
class sAM(nn.Module):  # For dataset RS, AR, and EXP
    ...
    def forward(self, x, mask=None):
        ...
        x = self.conv1(x)
        if mask is not None:
            x = x * mask  # clear non-medium pixel value
            x = x + ((1 - mask) * -1e8)  # eliminate affect of non-medium pixel in softmax activation

        if self.softmax:
          x_size = x.shape
          x = torch.reshape(x,(x_size[0],1,-1))
          x = F.softmax(x, dim=2)
          x = torch.reshape(x, x_size)*(x_size[2]*x_size[3] * 0.5)  # bate = 0.5
        else:
          x = self.sigmoid(x)
        return x


class EAPNet(nn.Module):  # For dataset RS, AR, and EXP

    def __init__(self, in_channels=1, up_mode='transpose', pooling=True):

        super(EAPNet, self).__init__()
    ...
    def forward(self, PAimg, mask=None):
